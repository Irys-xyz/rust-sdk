...
pub async fn deep_hash(chunk: DeepHashChunk) -> Result<Bytes, BundlrError> {
    match chunk {
        ...
        DeepHashChunk::Stream(mut s) => {
            ...
            let mut length = 0;
            while let Some(chunk) = s.as_mut().try_next().await.map_err(|_| BundlrError::NoBytesLeft)? {
                length += chunk.len();
                hasher.update(&chunk);
            }
            ...
            let tagged_hash = [sha384hash(tag.into()), Bytes::copy_from_slice(&hasher.finish())].concat();
...
pub async fn deep_hash_chunks(mut chunks: Vec<DeepHashChunk>, acc: Bytes) -> Result<Bytes, BundlrError> {
    if chunks.len() == 0 {
        return Ok(acc);
    };
