...
use bytes::Bytes;
use ed25519_dalek::{SecretKey, Keypair, PublicKey, Signer};
...
impl SolanaSigner {
    pub fn new(key: Keypair, public: Vec<u8>) -> SolanaSigner {
        SolanaSigner {
            key
        }
    }
    ...
    pub fn from_base58(s: &str) -> Self {
        ...
        let key: &[u8; 64] = k.as_slice().try_into().expect("Couldn't convert base58 key to bytes");
        let sc = SecretKey::from_bytes(&key[..32]).unwrap();
        let pubkey = PublicKey::from_bytes(&key[32..64]).unwrap();

        Self {
            key: Keypair { public: pubkey, secret: sc }
        }
    }
}
...
impl SignerTrait for SolanaSigner {
    ...
    fn sign(&self, message: bytes::Bytes) -> Result<bytes::Bytes, crate::error::BundlrError> {
        Ok(Bytes::copy_from_slice(&self.key.sign(&message[..]).to_bytes()))
    }
...
impl Verifier for SolanaSigner {
    fn verify(pk: Bytes, message: Bytes, signature: Bytes) -> Result<bool, crate::error::BundlrError> {
        todo!()
    }
}
...
mod tests {
    ...
    use crate::{SolanaSigner, Signer};

    #[test]
    fn test() {
        let message = &[
            110u8, 123, 209,  66, 178, 255, 153,  11,  45,
            235, 189, 244,  42,   9, 152, 192, 181, 183,
             40, 140, 216, 194, 141, 222, 128, 251, 237,
            133, 207, 198, 131,  71, 242, 117, 246, 186,
            189, 138, 117, 253,  31, 141, 117,  17, 179,
            138, 224, 131
          ];
