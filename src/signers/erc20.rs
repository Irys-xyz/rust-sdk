...
pub struct ERC20Signer {
    sec_key: SecretKey,
    pub_key: PublicKey,
}

impl ERC20Signer {
    pub fn new(sec_key: SecretKey) -> ERC20Signer {
        let secp = Secp256k1::new();
        let pub_key = PublicKey::from_secret_key(&secp, &sec_key);
        ERC20Signer { sec_key, pub_key }
    }
...
mod tests {
    ...
    use crate::{ERC20Signer, Signer, Verifier};
    ...
    fn should_create_signer() {
        ...
        ERC20Signer::new(secret_key);
        ERC20Signer::from_base58(base58_secret_key);
    }
    ...
    fn should_sign_and_verify() {
        ...
        let secret_key = SecretKey::from_slice(&[0xcd; 32]).expect("");
        let signer = ERC20Signer::new(secret_key);

        let sig = signer.sign(msg.clone()).unwrap();
        let pub_key = signer.pub_key();

        assert!(ERC20Signer::verify(pub_key, msg, sig).unwrap());
    }
}
